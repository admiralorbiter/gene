...
- **Standardized Premise Omission Semantics:** Every knockout ($do(X=0)$) is strictly executed as pure premise deletion from context. No `REVOKED` or status string is introduced, eliminating toxic keyword conservatism bias.
...
1. **Baseline Control ($\emptyset$):** $\{A, B, D, E\}$ all present $\implies$ Formal: $C = \text{PROTO\_X7}$.
2. **Single Knockouts (4):**
   - $\{A\}$: $\{B, D, E\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (via $DE$).
   - $\{B\}$: $\{A, D, E\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (via $DE$).
   - $\{D\}$: $\{A, B, E\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (via $AB$).
   - $\{E\}$: $\{A, B, D\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (via $AB$).
3. **Path Isolation Knockouts (2):**
   - $\{A, B\}$: $\{D, E\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (tests path $DE$ in isolation).
   - $\{D, E\}$: $\{A, B\}$ present $\implies$ Formal: $C = \text{PROTO\_X7}$ (tests path $AB$ in isolation).
4. **Cross-Path Minimal Hitting Sets (4):**
   - $\{A, D\}$: $\{B, E\}$ present $\implies$ Formal: $C \to \text{UNKNOWN}$ (both paths broken).
   - $\{A, E\}$: $\{B, D\}$ present $\implies$ Formal: $C \to \text{UNKNOWN}$.
   - $\{B, D\}$: $\{A, E\}$ present $\implies$ Formal: $C \to \text{UNKNOWN}$.
   - $\{B, E\}$: $\{A, D\}$ present $\implies$ Formal: $C \to \text{UNKNOWN}$.
