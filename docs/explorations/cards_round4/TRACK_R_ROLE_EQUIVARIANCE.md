...
- **Core Hypothesis:** In Exploration Round 3 Track H (KESTREL), the model sustained `PROTO_X7` on formally invalid cross-premise combinations ($\{B,D\}$ and $\{A,D\}$), with premise $D$ (`sector_lead`) appearing in 3 of 4 minimal coalitions. This indicates that `sector_lead` functions as a privileged natural-language authority cue allowing premature inference.
- **The Discriminating Contrast:**
  ...
  - We apply a **topology-preserving semantic intervention** swapping roles $A \leftrightarrow D$ ($A = \text{sector\_lead}, D = \text{manager}$):
    1. **Semantic-Role Hypothesis:** If the shortcut is driven by the semantic authority of `sector_lead`, the shortcut must **invert**: $\{A,E\}$ becomes behaviorally sufficient ($\to \text{PROTO\_X7}$), while $\{B,D\}$ collapses to $\text{UNKNOWN}$.
    2. **Graph-Slot / Position Hypothesis:** If the shortcut is driven by graph topology or position $D$, $\{B,D\}$ remains behaviorally sufficient even though $D$ is now `manager`.
...
We evaluate 3 representation conditions $\times$ 8 key lattice points:
