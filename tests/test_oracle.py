"""Unit and invariant tests for forward-chaining deterministic oracle."""
...
def test_golden_world_closure(golden_world: World):
    ...
    # Check derived amber protocol fact
    amber_id = compute_fact_id("VELORA", "uses_protocol", "PROTOCOL_AMBER")
    assert amber_id in oracle.closure_facts
    assert oracle.closure_facts[amber_id].source_type == "derived"


def test_golden_world_minimal_support_paths(golden_world: World):
    oracle = Oracle(golden_world)
    ...
    # Derived green fact requires {manager(VELORA, NERIN), reports_to(NERIN, TAL)}
    f2_id = compute_fact_id("NERIN", "reports_to", "TAL")
    green_id = compute_fact_id("VELORA", "uses_protocol", "PROTOCOL_GREEN")
    green_paths = oracle.get_support_paths(green_id)
    expected_support = sorted([f1_id, f2_id])
    assert green_paths == [expected_support]

    # Derived amber fact requires {located_in(VELORA, SECTOR_K)}
    f3_id = compute_fact_id("VELORA", "located_in", "SECTOR_K")
    amber_id = compute_fact_id("VELORA", "uses_protocol", "PROTOCOL_AMBER")
    amber_paths = oracle.get_support_paths(amber_id)
    assert amber_paths == [[f3_id]]
