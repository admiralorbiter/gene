"""Paired Authority and Privilege Scope Sidecar (Stage 7A.1)."""
...
def run_paired_authority_assay() -> dict[str, Any]:
    """Evaluate do(SourceClass = s_i) holding proposition P invariant under runtime-derived context.
    
    1. PLATFORM_ATTESTED -> ADMIT (Authorized ROOT_FACT)
    2. USER_DIRECT -> ADMIT for authorized scope
    3. THIRD_PARTY_QUOTED -> REJECT (Unauthenticated origin restricted to ATTESTATION_ONLY)
    4. MODEL_DERIVED -> REJECT as root fact (Privilege restricted to ATTESTATION_ONLY)
    """
    ...
    capability_registry = CapabilityPolicyRegistry({
        "platform_sensor": CapabilityPolicy("platform_sensor", frozenset(["device_status"]), ClaimPrivilege.ROOT_FACT, "SENSOR", "ROOT_NET_1"),
        "user_operator": CapabilityPolicy("user_operator", frozenset(["operator_task", "device_status"]), ClaimPrivilege.ROOT_FACT, "OPERATOR", "ROOT_USER"),
        "third_party_web": CapabilityPolicy("third_party_web", frozenset(["device_status"]), ClaimPrivilege.ATTESTATION_ONLY, "WEB", "ROOT_WEB"),
        "neural_model": CapabilityPolicy("neural_model", frozenset(["device_status"]), ClaimPrivilege.ATTESTATION_ONLY, "MODEL", "ROOT_MODEL"),
    })
    ...
    rec_4 = SourceRecord("rec_4", "Server Node 1 is Operational", CaptureProvenance("c4", "cot_step", 1, "h4"), ClaimedOrigin("cot_reasoner", "neural_model"), AuthenticatedOrigin("cot_reasoner", "KERNEL_LOCAL", True), 1)
